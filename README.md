...
| 设备类型   | 1          | 固定值：0x01                                                 |
| 设备地址   | 1          | 固定值：0x01                                                 |
| 校验和     | 2          | 该数据包所有字节的校验和，大端存储                           |
| 数据类型   | 1          | 表示传输数据包为按键包，固定值：0x00                         |
| 按键状态   | 2          | 大端存储，每个bit代表一个按键，该bit值为1时代表按下，为0时代表未按下。 |
...
最终数据包：0xFB，0x01，0x0A，0x01，0x01，0x01，0x28，0x00，0x00，0xC0



例如：在按键B和按键C都按下时，传输的数据包为：
...
最终数据包：0xFB，0x01，0x0A，0x01，0x01，0x01，0xC8，0x00，0x00，0xC0
...
| 设备地址   | 1          | 固定值：0x01                                   |
| 校验和     | 2          | 该数据包所有字节的校验和，大端存储             |
| 数据类型   | 1          | 表示传输数据包为摇杆包，固定值：0x01           |
| 摇杆数据   | 2          | 第一个字节为摇杆的x值，第二个字节表示摇杆的y值 |
...
| 设备地址         | 1          | 固定值：0x01                                                 |
| 校验和           | 2          | 该数据包所有字节的校验和，大端存储                           |
| 数据类型         | 1          | 表示传输数据包为摇杆包，固定值：0x02                         |
| 重力加速度计数据 | 6          | 大端存储，6个字节，依次代表重力加速度计的x，y，z值（各占俩个字节） |
